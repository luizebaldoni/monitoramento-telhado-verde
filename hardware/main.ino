...
//& Converte DeviceAddress (8 bytes) em string hex sem separadores
...
//& DS18B20 – sensores de temperatura do solo via OneWire (suporta múltiplos)
#define ONE_WIRE_BUS  27
OneWire oneWire(ONE_WIRE_BUS);
DallasTemperature ds18b20(&oneWire);
#define MAX_DS_SENSORS 3                // Suporte para até 3 sensores DS18B20 na mesma linha OneWire
DeviceAddress dsAddrs[MAX_DS_SENSORS];
int dsCount = 0;                          // quantidade detectada (limitada a MAX_DS_SENSORS)
...
bool sendSensorData(
  ...
  //& Documento JSON no formato esperado pela API
  ...
  //& Bloco de dados do(s) sensor(es) DS18B20 (array)
  JsonArray dsArray = sensors.createNestedArray("ds18b20");
  for (int i = 0; i < ds_count; i++) {
    JsonObject dsObj = dsArray.createNestedObject();
    dsObj["index"] = i;
    dsObj["temperature"] = isnan(ds_temps[i]) ? 0.0 : ds_temps[i];
    dsObj["unit"] = "celsius";
    dsObj["status"] = isnan(ds_temps[i]) ? "error" : "ok";
  }
  ...
  dht["unit_temp"] = "celsius";                             //envia em graus celsius
  dht["unit_humidity"] = "percent";                       //envia em porcentagem
  ...
  hcsr["unit"] = "cm";                                      // distacia em centimetros
  ...
  hl["raw_value"] = hl_raw;
  hl["unit"] = "percent";                                   // envia em porcentagem
  hl["status"] = "ok";
...
void setup() {
  ...
  //& Inicialização do sensor DS18B20 (varredura de dispositivos OneWire)
  ds18b20.begin();
  int found = ds18b20.getDeviceCount();
  Serial.print("DS18B20 devices detected: ");
  Serial.println(found);
  dsCount = found;
  if (dsCount > MAX_DS_SENSORS) dsCount = MAX_DS_SENSORS;
  for (int i = 0; i < dsCount; i++) {
    if (ds18b20.getAddress(dsAddrs[i], i)) {
      ds18b20.setResolution(dsAddrs[i], 12);                                // resolução máxima (12 bits)
      Serial.print("DS18B20["); Serial.print(i); Serial.print("] addr: ");
      Serial.println(deviceAddressToString(dsAddrs[i]));
    } else {
      Serial.print("Falha ao obter endereço DS18B20 index "); Serial.println(i);
    }
  }
  if (dsCount == 0) {
...
void loop() {
  ...
  timeClient.update();                          // Atualiza timeClient no início da amostragem para ter timestamp consistente
  String ts = formatIsoTimestamp(timeClient.getEpochTime());  // obtém timestamp ISO 8601
  Serial.print("Timestamp: "); Serial.println(ts);
  Serial.println("------------------------------");

  //& Leitura DS18B20 (temperatura do solo) - vários sensores
  ds18b20.requestTemperatures();                // solicita leitura de todos os sensores DS18B20
  float ds_temps[MAX_DS_SENSORS];
  for (int i = 0; i < MAX_DS_SENSORS; i++) ds_temps[i] = NAN;    // inicializa com NAN
  for (int i = 0; i < dsCount; i++) {                           // lê cada sensor
    float t = ds18b20.getTempC(dsAddrs[i]);           // obtém temperatura em °C
    if (t == DEVICE_DISCONNECTED_C) {                 // erro de leitura
      ds_temps[i] = NAN;                              // marca como NAN
      Serial.printf("DS18B20[%d] %s  Status: ERROR\n", i, deviceAddressToString(dsAddrs[i]).c_str());
    } else {
      ds_temps[i] = t;                                 // armazena temperatura válida
      Serial.printf("DS18B20[%d] %s  Temp: %.2f C\n", i, deviceAddressToString(dsAddrs[i]).c_str(), t);
    }
  }
  ...
  if (!isnan(hum) && !isnan(tDHT)) {                // leitura válida
    Serial.printf("DHT11      Temp: %.2f C | Umidade: %.2f %%\n", tDHT, hum);
  } else {
  ...
  Serial.printf("HL-69      Raw: %d | Solo: %.2f %%\n", hl_raw, soil_pct);

  //& Envia os dados coletados para a API FastAPI via HTTP POST (no formato do JSON esperado)
